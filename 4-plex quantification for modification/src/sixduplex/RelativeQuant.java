...
public class RelativeQuant {
    ...
    public List<String> transferString(List<String> pepVarMod){
        ...
        for(int i=0;i<n;i++){
            ...
            str1=pepVarMod.get(i).replace("M(ox)", "3");
            str1=str1.replace("H(ph)", "5");
            str1=str1.replace("K(ph)", "5");
            str1=str1.replace("R(ph)", "5");
            str1=str1.replace("S(ph)", "5");
            str1=str1.replace("T(ph)", "5");
            str1=str1.replace("Y(ph)", "5");
            
            str1=str1.replace("S(he)", "6");
    ...
    public void calculateLightHeavy(String peptidesequence, String peptidevarmod,int chargeprecursor,String dtafilename,HashMap<Double,Double> expmsms){
        ...
        ////to judge whether the ion exists for each labeling
        List<String> isExist=new ArrayList<>();
        List<String> firstTheoName=new ArrayList<>(allPep.get(0).getTheoMSMS().keySet());
        for(int i=0;i<firstTheoName.size();i++){
            boolean flags=true;
            for(int j=1;j<multiple;j++){
                if(!allPep.get(j).getTheoMSMS().containsKey(firstTheoName.get(i)))
                    flags=false;
            }
            if(flags)
                isExist.add(firstTheoName.get(i));
        }
        ////////////////////
        //theoretical ms/ms ion mz
        List<List<Double>> allTheoMz=new ArrayList<>();
        for(int i=0;i<multiple;i++){
            List<Double> oneTheoMz=new ArrayList<>();
            for(String key:isExist){
                oneTheoMz.add(allPep.get(i).getTheoMSMS().get(key));
            }
            allTheoMz.add(oneTheoMz);
            //System.out.println(oneTheoMz);
        }
        //theoretical ms/ms ion name
        List<String> theoName=new ArrayList<>();
        for(String key:isExist){
            theoName.add(key);
        }
    ...
    public List<Integer> findCommonIon(List<List<Double>> ionCount,List<List<Double>> alltheomz, HashMap<Double,Double> expmsms,String dtafilename){
       ...
       // System.out.println(alltheomz.get(0).size());
         //System.out.println(alltheomz.get(1).size());
          //System.out.println(alltheomz.get(2).size());
           //System.out.println(alltheomz.get(3).size());
                    ...
                    for(int j=0;j<expL;j++){
                        ...
                        for(int k=0;k<multiple;k++){
                            ...
                            if(Math.abs(temp-alltheomz.get(k).get(i))<threshold&&alltheomz.get(k).get(i)<mzThreshold){
                                ...
                                if(expmsms.get(temp)>commonIonIntensity.get(k))
